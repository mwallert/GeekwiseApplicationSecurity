...
// Static stuff
const order = document.querySelector('#post-order');
...
order.addEventListener('click', e => {
    ...
    if(e.target.value === 'ASC') {
        e.target.value = 'DESC';
        order = true;
    } else e.target.value = 'ASC';

    jQuery.post(`${_baseUrl}:${_port}/api/post/search`, { search: $('#search').val(), order: order }, function(data) {
        generatePosts(data);
    });
});

function getPosts(order) {
    clearEdit();
    const endPoint = `${_baseUrl}:${_port}/api/post${order ? '/ordered/posts' : ''}`
